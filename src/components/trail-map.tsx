...
export default function TrailMap({
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const hasRoute = routeLatLngs.length > 1;
      ...
      if (hasRoute) {
        ...
        parsedSections.forEach((s) => {
          if (Number.isFinite(s.lat) && Number.isFinite(s.lng)) bounds.extend([s.lat, s.lng]);
        });
  ...
  return (
    <div className={cn("space-y-2", className)}>
      <div className="overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800">
        <div ref={mapDivRef} className="h-[360px] w-full bg-zinc-100 dark:bg-zinc-900" />
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs text-zinc-600 dark:text-zinc-400">
        ...
        <span className="rounded-full border px-2 py-1">Start / End</span>
        <span className="rounded-full border px-2 py-1">Sections</span>
      </div>
